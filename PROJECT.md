...
- 이번 주 현황 표: **멤버 열 고정(모바일 폭 = 아바타 + 한글 3글자 + 여백, 긴 이름은 말줄임), 요일 영역만 가로 스크롤**, 멤버 × 월~일, ✓+시간(1시간 이상) / 회색 시간(미달) / -, 오늘 강조, 목표 기간 밖 날짜는 흐리게
...
| 2026-09-23 | 대시보드 이번 주 현황 — 멤버 열을 스크롤 영역에서 분리해 고정, 모바일 멤버 열 폭 축소(한글 3글자 + 여백), 요일 영역만 가로 스크롤 |
